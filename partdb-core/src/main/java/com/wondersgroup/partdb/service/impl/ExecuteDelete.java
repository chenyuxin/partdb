...
@Service("DELETE")
public class ExecuteDelete implements ExecuteSqlService {
	
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ExecuteDelete.class);
	...
	public PartDbExeResult<?> executeSql(String sql, SQLStatementParser parser) {
		...
		try {
			SQLDeleteStatement statement =  parser.parseDeleteStatement();
